...
export function IndustrySystemsPanel({
  ...
  return (
    <div className="grid min-h-[520px] min-w-0 gap-gutter lg:grid-cols-[0.42fr_1fr]">
            ...
            {products.map((product) => (
              <li key={product.slug}>
                <button
                  ...
                  className={cn(
                    "flex w-full min-w-0 items-start justify-between gap-3 border-b pb-2 text-left transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-warning-red",
      ...
      <div className="relative h-[min(720px,calc(100vh-340px))] min-h-[520px] min-w-0 overflow-hidden border border-graphite-muted bg-surface-container-low/90 shadow-[0_24px_80px_rgb(20_19_19_/_0.1)] backdrop-blur-xl dark:bg-graphite-muted/20 dark:shadow-none">
        <div
          ref={scrollAreaRef}
          onScroll={handleDetailScroll}
          className="scrollbar-none h-full min-w-0 scroll-pt-4 overflow-y-auto p-3 pr-5 sm:scroll-pt-6 sm:p-6 sm:pr-8"
        >
          <div className="min-w-0 space-y-5 pb-[620px]">
            {products.map((product) => (
              <article
                key={product.slug}
                ref={(node) => {
                ...
                className={cn(
                  "reticle-corners grid w-full min-w-0 max-w-full gap-4 border bg-surface-container-low/95 p-3 transition-[border-color,opacity,transform] duration-500 ease-out dark:bg-surface-container-low/60 sm:p-5 md:grid-cols-[minmax(0,220px)_minmax(0,1fr)] md:gap-5",
                  activeSlug === product.slug
              ...
              >
                ...
                <div className="flex min-w-0 flex-col justify-center">
                  ...
                  <Link
                    ...
                    className="mt-6 inline-flex max-w-full whitespace-normal break-words border border-warning-red px-4 py-3 text-center font-mono text-label-xs uppercase tracking-[0.14em] text-warning-red transition-colors hover:bg-warning-red hover:text-primary-container"
                  >
                    {openDetailLabel}
                  </Link>
                </div>

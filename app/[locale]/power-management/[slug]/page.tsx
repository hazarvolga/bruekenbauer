...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const family = powerManagementFamilies.find((item) => item.slug === slug);
  if (!family) return {};
      ...
      <section className="grid border-b border-graphite-muted lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="grid content-center px-margin-mobile py-16 md:px-margin-desktop">
          ...
          <div className="mt-10 flex flex-wrap gap-4">
            <TechnicalButton href={`/rfq?familySlug=${family.slug}`}>Request Quote</TechnicalButton>
            <TechnicalButton href="/intro" variant="ghost">
              Back to Power Stack
            </TechnicalButton>
          </div>
        </div>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            {family.targetApplications.map((application) => (
              <Link
                ...
                href={`/industries/${slugify(application.applicationName)}`}
